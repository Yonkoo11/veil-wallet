...
export function UnshieldScreen() {
  ...
  const handleUnshield = () => {
    if (!amount || !isValidAddress || !walletId) return;
    broadcastStarted.current = false;
    setPhase("proving");
    generateProof("unshield", {
      chain: "polygon",
      walletId,
  ...
  if (phase !== "form") {
    return (
      <div className="flex flex-col min-h-dvh px-4 py-6">
        ...
        <TxProgress
          title={`Unshielding ${amount} ${selectedToken.symbol}`}
          progress={progress}
          status={
  ...
  return (
    <div className="flex flex-col min-h-dvh px-4 py-6">
      ...
      <p className="text-sm text-neutral-500 mb-6 fade-in delay-1">
        Withdraw private tokens to any public address. The destination receives
        tokens from the Railgun contract with no on-chain link to you.
      </p>

      <label className="text-xs text-neutral-500 mb-2 fade-in delay-2">
